...
def getCountsAndAverages(IDandRatingsTuple):
    ...
    """
    id, rating_tuple = IDandRatingsTuple
    return (id, (len(rating_tuple), sum(rating_tuple) * 1.0 / len(rating_tuple)))
...
movieIDsWithRatingsRDD = (ratingsRDD
                          .map(lambda r: (r[1], r[2])).groupByKey())
...
def computeError(predictedRDD, actualRDD):
    ...
    actualReformattedRDD = actualRDD.map(lambda p: ((p[0], p[1]), p[2]))

    # Compute the squared error for each matching entry (i.e., the same (User ID, Movie ID) in each
    # RDD) in the reformatted RDDs using RDD transformtions - do not use collect()
    squaredErrorsRDD = (predictedReformattedRDD
                        .join(actualReformattedRDD).map(lambda p: (p[0], (p[1][0] - p[1][1]) ** 2)))

    # Compute the total squared error - do not use collect()
    totalError = squaredErrorsRDD.map(lambda p: p[1]).sum()
...
validationForPredictRDD = validationRDD.map(lambda p: (p[0], p[1]))
...
myModel = ALS.train(trainingRDD, bestRank, seed=seed, iterations=iterations,
                      lambda_=regularizationParameter)
testForPredictingRDD = testRDD.map(lambda p: (p[0], p[1]))
...
# TODO: Replace <FILL IN> with appropriate code
trainingAvgRating = trainingRDD.map(lambda p: p[2]).mean()
print 'The average rating for movies in the training set is %s' % trainingAvgRating

testForAvgRDD = testRDD.map(lambda p: (p[0], p[1], trainingAvgRating))
...
print 'My movie ratings: %s' % myRatingsRDD.take(2)
...
# TODO: Replace <FILL IN> with appropriate code
predictedTestMyRatingsRDD = myRatingsModel.predictAll(trainingWithMyRatingsRDD.map(lambda p: (p[0], p[1])))
...
predictedRatingsRDD = myRatingsModel.predictAll(myUnratedMoviesRDD.map(lambda p: (p[0], p[1])))
...
movieCountsRDD = movieIDsWithAvgRatingsRDD.map(lambda p: (p[0], p[1][0]))
